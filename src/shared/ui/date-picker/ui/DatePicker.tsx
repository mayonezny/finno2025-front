...
import type { DatePickerRef, DatePickerProps } from '../model/types';
...
export const DatePicker = forwardRef<DatePickerRef, DatePickerProps>((props, ref) => {
  ...
  const formatRange = (range: [Date | null, Date | null] | null): [string, string] =>
    range ? [formatDate(range[0]), formatDate(range[1])] : ['', ''];

  const formatRangeTime = (range: [Date | null, Date | null] | null): [string, string] =>
  ...
  const getDateFromISO = (iso: string | null): Date | null => (iso ? new Date(iso) : null);
  ...
  const [inputValue, setInputValue] = useState<string | [string, string]>(
    type === 'single'
      ...
      : formatRange(
          Array.isArray(value) ? (value.map(getDateFromISO) as [Date | null, Date | null]) : null,
        ),
  );
  ...
  // EVENT HANDLERS
  // --------------------------
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>, index = 0) => {
    ...
    if (type === 'single') {
      ...
      onChange(
        finalDates[0] && finalDates[1] ? [toISO(finalDates[0])!, toISO(finalDates[1])!] : null,
      );
      if (parsed[index]) {
        setCurrentMonth(new Date(parsed[index]!.getFullYear(), parsed[index]!.getMonth(), 1));
      }
    }
    setTouched(true);
  };

  const handleTimeChange = (e: React.ChangeEvent<HTMLInputElement>, index = 0) => {
    const val = e.target.value;
  ...
  const handleDateSelect = (date: Date) => {
    ...
    if (type === 'single') {
      ...
      const [start, end] = (inputValue as [string, string]).map(parseDateString) as [
        Date | null,
        Date | null,
      ];
      ...
      if (newRange[0]) {
        setCurrentMonth(new Date(newRange[0].getFullYear(), newRange[0].getMonth(), 1));
      }
    }
  };
  ...
  const generateCalendarDays = (): JSX.Element[] => {
    ...
    const offset =
      weekStart === 1 ? (startDayOfWeek === 0 ? 6 : startDayOfWeek - 1) : startDayOfWeek;
    ...
    for (let d = 1; d <= lastDay.getDate(); d++) {
      ...
      const disabledDay = (minDate && dayDate < minDate) || (maxDate && dayDate > maxDate);
      const selected =
        (type === 'single' &&
          typeof value === 'string' &&
          isSameDay(getDateFromISO(value)!, dayDate)) ||
        (Array.isArray(value) && value.some((v) => v && isSameDay(getDateFromISO(v)!, dayDate)));
    ...
    if (!value) {
      ...
      const dates =
        type === 'single'
          ...
          : ((value as [string, string]).map(getDateFromISO) as [Date | null, Date | null]);
      setInputValue(
        type === 'single' ? formatDate(dates as Date) : formatRange(dates as [Date, Date]),
      );
      if (withTime) {
        setInputTime(
          type === 'single' ? formatTime(dates as Date) : formatRangeTime(dates as [Date, Date]),
        );
      }
      if (dates instanceof Date && !isNaN(dates.getTime())) {
        setCurrentMonth(new Date(dates.getFullYear(), dates.getMonth(), 1));
      } else if (Array.isArray(dates) && dates[0] && !isNaN(dates[0].getTime())) {
        setCurrentMonth(new Date(dates[0].getFullYear(), dates[0].getMonth(), 1));
      }
    }
  ...
  useImperativeHandle(ref, () => ({
    validate: () => {
      ...
      if (type === 'single') {
        ...
        const parsed = (inputValue as [string, string]).map(parseDateString) as [
          Date | null,
          Date | null,
        ];
        setRangeError(parsed);
  ...
  // --------------------------
  return (
    <div ref={wrapperRef} className={`date-picker ${className} ${disabled ? 'disabled' : ''}`}>
      {label && (
        <label className="date-picker-label body-regular">
          {label}
      ...
      {isOpen && (
        <div className="calendar-popup">
          <div className="calendar-header">
            <button type="button" className="month-nav prev" onClick={() => handleMonthChange(-1)}>
              {' '}
              {'<'}{' '}
            </button>
            ...
            <button type="button" className="month-nav next" onClick={() => handleMonthChange(1)}>
              {' '}
              {'>'}{' '}
            </button>
          </div>
